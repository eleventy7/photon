...
{
    public static final int MESSAGE_TYPE = MessageTypes.MARKET_PRICE;
    ...
    private static final int VENUE_OFFSET = PRICE_OFFSET + Double.BYTES;
    ...
    private String pair; //assuming max 8 char for USDT/USD

    //pojo if needed; could convert to flyweight if needed later.
    public static MarketPrice fromDirectBuffer(DirectBuffer source, int offset) {
        final MarketPrice result = new MarketPrice();
